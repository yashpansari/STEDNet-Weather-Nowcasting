...
class ConvLSTMModel(nn.Module):
    def __init__(self, hid_chan, in_chan=1, num_layers=2, use_last = True):
        ...
        chans = [in_chan] + [hid_chan] * num_layers
        ...
        self.decoder_CNN = nn.Conv2d(in_channels=hid_chan,
                                     out_channels=in_chan,
                                     kernel_size=(3, 3),
                                     padding=(1, 1))     
        self.save_fp = "drive/MyDrive/Climate-Hack-2022/Yash/seq2seq/eval_model"
        if use_last:
          self.load_params()
    ...
    def forward(self, inp_imgs, teacher_force=1, teacher_force_start=12):
        ...
        for t in range(self.frame_cnt):
            ...
            pred = self.decoder_CNN(final_hid) # [batch, in chan, img h, img w]

...
def flow_of_batch(batch_x, batch_y):
  ...
  for i in range(len(batch_x)):
    ...
    flow = torch.Tensor(cv.calcOpticalFlowFarneback(prev=x1,next=x2, flow=None, pyr_scale=0.5, levels=3, winsize=5, iterations=10, poly_n=5, poly_sigma=1.2, flags=cv.OPTFLOW_FARNEBACK_GAUSSIAN))
    if flows is None:
      flows = flow.unsqueeze(dim=0)
    else:
      flows = torch.cat((flows, flow.unsqueeze(dim=0)), dim=0)
  return torch.reshape(flows, (len(batch_x), 2, 128, 128))
...
class ConvLSTMModel(nn.Module):
    def __init__(self, hid_chan, in_chan=1, num_layers=2, use_last = True):
        ...
        self.decoder_CNN = nn.Conv2d(in_channels=hid_chan + 2,
                                     ...
                                     kernel_size=(3, 3),
                                     padding=(1, 1))     
        self.save_fp = "drive/MyDrive/Climate-Hack-2022/Yash/seq2seq/OF+DropoutOnlyLSTM+WD+deep"
        if use_last:
          self.load_params()
    ...
    def forward(self, inp_imgs, teacher_force=1, teacher_force_start=12):
        ...
        flows = torch.zeros((batch_size, 2, h, w))
        weight = 1
        ...
        for t in range(self.frame_cnt):
            ...
            flows = (flows/2 + flow_of_batch(x1,x2)) / weight
            weight += (1/2)**t
    ...
    def ultimate_pred(self, inp_imgs, pred_len=24):
        ...
        z = torch.zeros([batch, pred_len - 1, h, w])
    ...
    def load_params(self):
      self.load_state_dict(torch.load(self.save_fp, map_location=torch.device('cpu')))